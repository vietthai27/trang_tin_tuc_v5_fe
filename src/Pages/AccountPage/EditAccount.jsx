...
import { closeModalEditAccount, editAccountRequest, getAccountByIdRequest, getBankListRequest } from "./redux";

function EditAccount() {
    const dispatch = useDispatch();
    const { itemId, accountById, listBanks, bankIdEdit } = useSelector(state => state.accountList);
    ...
    useEffect(() => {
        if (itemId) {
            dispatch(getAccountByIdRequest(itemId));
            dispatch(getBankListRequest())
        }
    }, [dispatch, itemId]);

    useEffect(() => {
        if (accountById && !image) {
            setAccountNumber(accountById.accountNumber || "");
            setPreview(accountById.qrCodeUrl || null);
            setBankId(bankIdEdit)
        }
    }, [accountById, image]);
    ...
    const handleSubmit = () => {
        ...
        dispatch(closeModalEditAccount());
    };
